...
| D8 | Phase 2 (drip) built. `sql/email_drip.sql` applied (candidate/claim/finish/track/event RPCs, `drip_done_at`). The day-0 welcome goes out at confirm time, and the daily cron sends the rest. The track choice is a one-click page (`/subscribe/choose/<track>/`); the 2-minute guard was dropped because people read the welcome right after confirming. Postmark link tracking is on for drip mail (click-rate metric) and off for the confirm email. Amazon Attribution uses an `email` channel with ad group `email` (per-track and per-step detail stays in the UTM tags on site links) | 2026-09-24 |
